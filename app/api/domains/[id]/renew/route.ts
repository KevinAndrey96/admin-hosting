...
export async function POST(
  ...
  try {
    ...
    const currentDate = new Date(domain.nextBillingDate);
...
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Dominio renovado</title></head>
<body style="margin:0;padding:0;font-family:sans-serif;background:#f4f4f5;">
  <table width="100%" cellspacing="0" cellpadding="0" style="background:#f4f4f5;">
    <tr><td align="center" style="padding:40px 20px;">
      <table width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        <tr><td style="padding:24px 32px;">
          ...
          <h2 style="margin:0 0 16px;font-size:18px;color:#1a1a2e;">Dominio renovado</h2>
          <p style="margin:0 0 12px;font-size:15px;line-height:1.6;color:#4a4a68;">
            Hola <strong>${domain.user.fullName}</strong>,
          </p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#4a4a68;">
            Te informamos que tu dominio <strong>${domain.fqdn}</strong> ha sido renovado correctamente por un año adicional.
          </p>
          ...
          <p style="margin:0;font-size:14px;line-height:1.6;color:#6c757d;">
            Tu dominio continúa activo sin interrupciones. Si tienes alguna pregunta, no dudes en contactarnos.
          </p>
        </td></tr>
    ...
    await sendEmail({
      ...
      subject: `[${companyName}] Tu dominio ha sido renovado - ${domain.fqdn}`,
      html,
    });

    return NextResponse.json({
      message: 'Dominio renovado correctamente. Se ha enviado un correo al cliente.',
      nextBillingDate: newDate.toISOString(),
    });
  } catch (error) {
    console.error('Domain renew error:', error);
    return NextResponse.json(
      { error: 'Error al renovar el dominio' },
      { status: 500 }
    );
  }
}
